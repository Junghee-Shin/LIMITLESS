...
public class SearchRecipe2_2FXController implements Initializable {
	...
	public void  handlerDish01Btn(ActionEvent dish01Event){
		...
		try {
			...
			//외부 css로딩
			searchRecipe3_2Fx.getStylesheets().add(getClass().getResource("app.css").toString()); //로딩여부 확인
	...
	public void  handlerDish02Btn(ActionEvent dish02Event){
		...
		try {
			...
			//외부 css로딩
			searchRecipe3_3Fx.getStylesheets().add(getClass().getResource("app.css").toString()); //로딩여부 확인
